...
<ul class="nav nav-pills mb-4 gap-2" id="pills-tab" role="tablist">
    <li class="nav-item" role="presentation">
        <a href="{{ url('/admin/tambah-barang/html') }}" class="nav-link bg-white text-muted border font-weight-bold" style="border-radius: 10px; padding: 12px 20px;">
            <i class="mdi mdi-table me-1"></i> HTML Table
        </a>
    </li>
    <li class="nav-item" role="presentation">
        <a href="{{ url('/admin/tambah-barang/datatables') }}" class="nav-link active font-weight-bold" style="border-radius: 10px; padding: 12px 20px;">
    ...
    <div class="card-body p-4">
        <div class="table-responsive">
            <table class="display w-100" id="myTable">
                ...
                <tbody>
                    {{-- Dynamically filled by DataTables --}}
                </tbody>
            </table>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    $('#formTambah').submit(function(e) {
        ...
        setTimeout(function() {
            ...
            table.row.add([
                `<span class="badge bg-dark font-weight-bold" style="border-radius: 6px;">#${idCounter++}</span>`,
                `<span class="font-weight-bold text-dark">${nama}</span>`,
                `<span class="badge bg-soft-success text-success font-weight-bold" style="border-radius: 8px; padding: 8px 12px;">Rp ${Number(harga).toLocaleString('id-ID')}</span>`
            ]).draw();
